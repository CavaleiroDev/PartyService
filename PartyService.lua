...
local Errors = {
	...
	[426] = "[426] Deprecated function - %s",
	[426.1] = "[426] Deprecated function - it is recommended to change %s to %s"
}
...
function module:AddPlayer(Player: Player, Party: "PartyTable or string")
...
function module:GetCurrentPartyInfo()
	if IsPartyServerValue == true then
		return CurrentPartyInfo
	end
end

function module:GetCurrentPartyData()
	if IsPartyServerValue == true then
...
if RunService:IsServer() then
	if game.PrivateServerId ~= "" then
		...
		if y then
			...
			spawn(function() -- the event doesn't activate without this, don't ask me why
